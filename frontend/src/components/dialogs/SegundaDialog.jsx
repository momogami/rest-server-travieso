...
import { obtenerPrendasSegunda, agregarSegunda } from "../../services/services";

const SegundaDialog = ({ isOpen, onClose, onError, onSuccess }) => {
  ...
  const fetchOptions = async () => {
    const response = await obtenerPrendasSegunda();
  ...
  const onSubmit = async () => {
    try {
      const response = await agregarSegunda({
        ropa: selectedPrenda,
        cantidad: cantidad,
      });
